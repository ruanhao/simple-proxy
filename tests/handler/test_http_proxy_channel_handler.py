...
Cache-Control: no-cache\r\n\r\n"""
    ...
    def test_parsing_proxy_info_failed(self, mocker, ctx_mocker):
        handler = HttpProxyChannelHandler(EventLoopGroup())
        ctx_mocker.channel.return_value.channelinfo.return_value.peername = raddr
        ...
        ctx_mocker.write.assert_called_once_with(b'HTTP/1.1 405 Method Not Allowed\r\n\r\n')

    def test_credential_failed(self, mocker, ctx_mocker):
...
Cache-Control: no-cache\r\n\r\n"""
    ...
    def test_https_proxy(self, mocker, ctx_mocker):
        handler = HttpProxyChannelHandler(EventLoopGroup(), proxy_username="cisco", proxy_password="juniper")
        ctx_mocker.channel.return_value.channelinfo.return_value.peername = raddr
        ctx_mocker.channel.return_value.id.return_value.peername = "channel-1"
        mocker.patch(
...
Proxy-Connection: Keep-Alive\r\n\r\n"""
    ...
    def test_https_proxy_with_transform(self, mocker, ctx_mocker):
...
Proxy-Connection: Keep-Alive\r\n\r\n"""
    ...
    def test_channel_read_case_no_auth_required_but_providing(self, mocker, ctx_mocker):
        handler = HttpProxyChannelHandler(EventLoopGroup())
        ctx_mocker.channel.return_value.channelinfo.return_value.peername = raddr
        ctx_mocker.channel.return_value.id.return_value.peername = "channel-1"
        mocker.patch(
